...
In your F<Makefile.PL>: (Recommended Usage)
    ...
    install_script  'script/myscript';
...
any Perl installation version 5.004 or newer.
...
configure_requires: key in the bundled F<META.yml> file.

Although most of META.yml is considered advisory only, a L<CPAN> client
...
always reliable, and still cannot be relied upon.
...
Perl packages list, or displayed on the L<http://search.cpan.org/> website,
you just want them along for the ride.
...
The F<inc>, F<t> and F<share> (if install_share is used) directories are
automatically C<no_index>'ed for you if found and do not require an
explicit command.
